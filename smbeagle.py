...
verbosity_level_group = parser.add_mutually_exclusive_group()
parser.add_argument("-t", "--target", required=True,
                    metavar="", help="Takes single IP, CIDR or file.")
parser.add_argument("-m", "--mode", metavar="",
                    help="Host discovery options: ICMP, Port, Skip")
parser.add_argument("-e", "--enumerate", metavar="",
...
def error_handle(cmd_out):
    ERRORS = ["NT_STATUS_CONNECTION_REFUSED", "NT_STATUS_INVALID_NETWORK_RESPONSE", "NT_STATUS_INVALID_PARAMETER", "NT_STATUS_UNSUCCESSFUL", "NT_STATUS_IO_TIMEOUT",
              "NT_STATUS_ACCESS_DENIED", "NT_STATUS_LOGON_FAILURE", "NT_STATUS_REVISION_MISMATCH", "COULD NOT CONNECT", "NT_STATUS_HOST_UNREACHABLE", "no servers could be reached"]
    val = False
    for error in ERRORS:
        if error in cmd_out:
...
def get_nullsessions(target):
    ...
    result = run(rpc_command_lsaquery, stdout=PIPE, stderr=PIPE,
    ...
    if has_error != False:
        logger.red_indent(
            'Failed to authenticate with null sessions to {}'.format(logger.RED(target)))
        return False
    elif has_error == False:
        logger.green_indent(
            'Successfully authenticated with null sessions to {}'.format(logger.GREEN(target)))
        return True
